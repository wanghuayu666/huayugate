        ...
        if proxy_auth_enabled():
            ...
            if not check_credentials(username, password):
                client.sendall(
                    ...
                    b"Proxy-Authenticate: Basic realm=\"huayu Proxy\"\r\n"
                    b"Content-Length: 0\r\n\r\n"
                )
                return
...
class ProxyServerHandle:
    ...
    def _bind(self) -> socket.socket:
        ...
        try:
            ...
            print(f"HTTP/SOCKS5 proxy listening on {self.host}:{self.port}", flush=True)
            return server
        except Exception as e:
            if server is not None:
                try:
                    ...
                    pass
            if is_ipv6 and self.host in ("::", ""):
                print(f"[警告] 绑定 IPv6 {self.host}:{self.port} 失败 ({e})，正在尝试回退至 IPv4 0.0.0.0 ...", flush=True)
                try:
                    ...
                    server.listen(256)
                    server.settimeout(0.5)
                    print(f"HTTP/SOCKS5 proxy listening on 0.0.0.0:{self.port} (仅 IPv4)", flush=True)
                    return server
                except Exception as ex:
                    import vpn_utils
                    diag = vpn_utils.diagnose_local_obstructions(self.port, host="0.0.0.0")
                    diag_msg = diag[1] if diag else str(ex)
                    raise RuntimeError(f"[ERROR] Failed to start HTTP/SOCKS5 proxy on 0.0.0.0:{self.port}: {diag_msg}") from ex
            if is_ipv6 and self.host == "::1":
                print(f"[警告] 绑定 IPv6 {self.host}:{self.port} 失败 ({e})，正在尝试回退至 IPv4 127.0.0.1 ...", flush=True)
                try:
                    server = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                    server.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
                    ...
                    server.listen(256)
                    server.settimeout(0.5)
                    print(f"HTTP/SOCKS5 proxy listening on 127.0.0.1:{self.port} (仅 IPv4)", flush=True)
                    return server
                except Exception as ex:
                    import vpn_utils
                    diag = vpn_utils.diagnose_local_obstructions(self.port, host="127.0.0.1")
                    diag_msg = diag[1] if diag else str(ex)
                    raise RuntimeError(f"[ERROR] Failed to start HTTP/SOCKS5 proxy on 127.0.0.1:{self.port}: {diag_msg}") from ex

            import vpn_utils
            diag = vpn_utils.diagnose_local_obstructions(self.port, host=self.host)
            diag_msg = diag[1] if diag else str(e)
            raise RuntimeError(f"[ERROR] Failed to start HTTP/SOCKS5 proxy on {self.host}:{self.port}: {diag_msg}") from e
        ...
        while not self.stop_event.is_set():
            try:
                ...
                if self.stop_event.is_set():
                    return
                print("[ERROR] Proxy accept failed: socket closed unexpectedly", flush=True)
                time.sleep(0.5)
                continue
            except Exception as e:
                if self.stop_event.is_set():
...
def start_proxy_server_in_thread(host: str, port: int) -> ProxyServerHandle:
    handle = ProxyServerHandle(host, port)
    handle.start()
    return handle
