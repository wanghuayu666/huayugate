...
def current_proxy_port() -> int:
    return active_listen_port or LOCAL_PROXY_PORT

def proxy_connect_hosts() -> list[str]:
    if LOCAL_PROXY_HOST == "::":
        return ["::1", "127.0.0.1"]
    if LOCAL_PROXY_HOST == "0.0.0.0":
        return ["127.0.0.1"]
    if LOCAL_PROXY_HOST in ("",):
        return ["::1", "127.0.0.1"]
    if ":" in LOCAL_PROXY_HOST:
        return [LOCAL_PROXY_HOST, "127.0.0.1"]
    return [LOCAL_PROXY_HOST]

def proxy_url_hosts() -> list[str]:
    return [f"[{host}]" if ":" in host else host for host in proxy_connect_hosts()]

def rebind_proxy_gateway(port: int, reason: str = "") -> bool:
    global proxy_gateway_handle, active_listen_port
    target_port = bounded_int(port, LOCAL_PROXY_PORT, 1024, 65535)
    with proxy_gateway_lock:
        if proxy_gateway_handle and proxy_gateway_handle.is_running() and active_listen_port == target_port:
            return True

        old_handle = proxy_gateway_handle
        proxy_gateway_handle = None
        if old_handle:
            old_handle.stop()

        try:
            proxy_gateway_handle = proxy_server.start_proxy_server_in_thread(LOCAL_PROXY_HOST, target_port)
            active_listen_port = target_port
            _proxy_display = f"[{LOCAL_PROXY_HOST}]" if ":" in LOCAL_PROXY_HOST else LOCAL_PROXY_HOST
            set_state(active_listen_port=target_port, local_proxy=f"http://{_proxy_display}:{target_port}")
            if reason:
                log_to_json("INFO", "Proxy", f"代理监听端口已重绑为 {target_port}: {reason}")
            return True
        except Exception as exc:
            active_listen_port = 0
            set_state(active_listen_port=0, proxy_ok=False, proxy_error=f"代理监听端口 {target_port} 启动失败: {exc}")
            print(f"[代理重绑失败] 端口 {target_port} 启动失败: {exc}", flush=True)
            log_to_json("ERROR", "Proxy", f"代理监听端口 {target_port} 启动失败: {exc}")
            return False

def wait_for_proxy_gateway(port: int, timeout_seconds: float = 15.0) -> bool:
...
def node_has_datacenter_asn(node: dict[str, Any]) -> bool:
    text = " ".join(
        str(node.get(key) or "")
        for key in ("asn", "as_name", "owner")
    ).lower()
    return any(keyword in text for keyword in DATACENTER_ASN_KEYWORDS)
...
def is_residential_node(node: dict[str, Any]) -> bool:
    ...
    if ip_type not in ("residential", "mobile"):
        return False
    if quality in ("datacenter", "hosting", "proxy"):
        return False
...
def apply_residential_metadata(nodes: list[dict[str, Any]], previous_nodes: list[dict[str, Any]] | None = None) -> list[dict[str, Any]]:
    ...
    for node in nodes:
        ...
        preferred_port = parse_int(previous.get("assigned_port")) or parse_int(node.get("assigned_port"))
        if valid_residential_port(preferred_port) and preferred_port not in used_ports:
            assigned_ports[node_id] = preferred_port
            used_ports.add(preferred_port)

    next_port = RESIDENTIAL_PORT_BASE
    for node in nodes:
        node_id = str(node.get("id") or "")
        if not node.get("is_residential"):
            continue
        if node_id not in assigned_ports:
            while next_port <= 65535 and (next_port in used_ports or not valid_residential_port(next_port)):
                next_port += 1
            assigned_ports[node_id] = next_port if next_port <= 65535 else 0
            used_ports.add(assigned_ports[node_id])
            next_port += 1
...
def active_proxy_port_for_node(node: dict[str, Any] | None) -> int:
    if node and node.get("is_residential"):
        assigned = parse_int(node.get("assigned_port"))
        if valid_residential_port(assigned):
            return assigned
...
def generate_openclash_subscription(
    ...
    lines = ["proxies:" if residential_nodes else "proxies: []"]
    names: list[str] = []
    used_names: set[str] = set()
    for node in residential_nodes:
        base_name = clash_node_name(node)
        name = base_name
        suffix = 2
        ...
        lines.append(f"    server: {yaml_quote(server_host)}")
        lines.append(f"    port: {parse_int(node.get('assigned_port'))}")
        if username is not None and password is not None:
            lines.append(f"    username: {yaml_quote(username)}")
            lines.append(f"    password: {yaml_quote(password)}")
        lines.append("    udp: true")

    lines.extend([
        "proxy-groups:",
        f"  - name: {yaml_quote(OPENCLASH_GROUP_NAME)}",
        "    type: url-test",
        f"    url: {yaml_quote(OPENCLASH_HEALTHCHECK_URL)}",
        f"    interval: {OPENCLASH_URL_TEST_INTERVAL}",
        "    tolerance: 50",
        "    proxies:",
    ])
    if names:
    ...
    lines.extend([
        "rules:",
        f"  - {yaml_quote('MATCH,' + OPENCLASH_GROUP_NAME)}",
        "",
    ])
    return "\n".join(lines)

def host_for_subscription(raw_host: str | None) -> str:
    host = (raw_host or "").split(",", 1)[0].strip()
    if not host:
        return "127.0.0.1"
    if host.startswith("["):
        ...
        if killed_pids:
            ...
            for pid in killed_pids:
                try:
                    ...
                    pass
            print(f"[Cleanup] Terminated huayu OpenVPN processes: {killed_pids}", flush=True)
    except Exception as e:
    ...
    with lock:
        ...
        if config_to_delete:
            try:
                ...
                pass
    rebind_proxy_gateway(LOCAL_PROXY_PORT, "无活动 VPN 节点，回到默认代理端口")
...
def connect_node(node_id: str) -> str:
    ...
    try:
        ...
        if not wait_for_proxy_gateway(listen_port, timeout_seconds=10):
            raise RuntimeError(f"代理监听端口 {listen_port} 启动后未能连通")
        ...
        set_state(active_openvpn_node_id=node_id, is_connecting=False, last_check_message=f"Connected {node_id}", active_node_latency=latency_str, active_listen_port=listen_port)
  ...
  <div class="btn-group">
    ...
    <div class="dropdown">
      ...
      <div id="github_dropdown" class="dropdown-content">
        <a href="https://github.com/wanghuayu666/huayugate" target="_blank">正式版</a>
        <a href="https://github.com/wanghuayu666/huayugate/tree/bate" target="_blank">测试版</a>
      </div>
    </div>
    ...
    def do_GET(self) -> None:
        ...
        if effective_path in ("/", "/index.html"):
            ...
            proxy_gateway_status = {
                ...
                "details": f"监听地址: {LOCAL_PROXY_HOST}:{proxy_port}",
                "error": proxy_err
            }
...
def main() -> None:
    ...
    rebind_proxy_gateway(LOCAL_PROXY_PORT, "服务启动")
    
    # Wait for the gateway to officially start
    print("[网关] 正在启动代理网关...", flush=True)
    gateway_ready = wait_for_proxy_gateway(LOCAL_PROXY_PORT, timeout_seconds=15)
    ...
    print(f"Proxy: http://{LOCAL_PROXY_HOST}:{current_proxy_port()}", flush=True)
